...
for it in range(epochs):
    ...
    for xsample, ysample in zip(X_shuffled, y_shuffled):
        ...
        gradient = trqrnn.grad_BL_fd(params, xsample, ysample, Nwout=Nwout, client=None)
    ...
    if Eval < best_Eval:
        ...
        best_Eval = Eval
        best_gradient = trqrnn.grad_BL_fd(params, xsample, ysample, Nwout, client=None)
        np.savetxt('.grad_best_'+job_id+'.dat', best_gradient)
    
    if it in checkpoints:
        np.savetxt('.param_'+str(it)+'_'+job_id+'.dat', params)
        gradient = trqrnn.grad_BL_fd(params, xsample, ysample, Nwout, client=None)
