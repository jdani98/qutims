...
#### DATA PREPARATION  ##################################################################
fname = '../data_vdp_mu_1_3_del_5_16_ab_100_1000p.dat' # INPUT
data = np.loadtxt(fname)

# QRNN size and layers
nE = 2; nM = 3; nL = 5; nx = 3

x_data = np.array([[data[i,1],data[i,2]] for i in range(len(data[:,1]))]) # 2D-array! Include data encoding in 2 qubits
...
#fname_init = 'param0_vdp1_EMCZ2_adam_[ID].dat'
#param0 = np.loadtxt(fname_init)[:,0]

rnd_seed = 13244#int(job_id) # !!!
...
with open(logfile, 'a') as f:
    ...
    f.write('\n')
    #f.write('nshots %i' %0) #!!! indicate NUMBER OF SHOTS
    #f.write('\n')
...
trqrnn = train_qrnn(nT,nE,nM,nL,nx, rseed=rnd_seed)

for it in range(epochs):
    
    if shuffle:
        indices = np.random.permutation(Ntr)
        with open('.shuffle_indices_'+str(rnd_seed)+'.csv', 'a') as file:
    ...
    for xsample, ysample in zip(X_shuffled, y_shuffled):
        ...
        gradient = trqrnn.grad_BL_fd(params, xsample, ysample, Nwout=Nwout, client=client)
    ...
    if Eval < best_Eval:
        ...
        best_Eval = Eval
        best_gradient = trqrnn.grad_BL_fd(params, xsample, ysample, Nwout, client=client)
        np.savetxt('.grad_best_'+job_id+'.dat', best_gradient)
    
    if it in checkpoints:
        np.savetxt('.param_'+str(it)+'_'+job_id+'.dat', params)
        gradient = trqrnn.grad_BL_fd(params, xsample, ysample, Nwout, client=client)
