...
fname_x = '../data_vdp_mu_2_del_15_ab_100_1000p.dat' # INPUT
...
random.seed(1) # !!! must be the same as in training
...
with open('optimisations_info_vdp1.csv', 'a') as file:
    ...
    writer.writerow([None,info['Job ID'], int(info['Nodes'])*int(info['Cores per node']), info['Memory Utilized'], info['CPU Utilized'], info['Job Wall-clock time'],
                     Npoints, nT,Nwout,nE,nM,nL,nx,Nparam,'Adam',itcount,None,None,'True',shuffle,LR,nshots,
